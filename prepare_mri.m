function prepare_mri(mri_path,meg_file,save_path, redo_coreg, resolution)
...
%   Detailed explanation goes here
    ...
    for i = 1:length(files)
        if endsWith(files(i).name,['.L.midthickness.' resolution 'k_fs_LR.surf.gii'])
            filename = fullfile(mri_path,'workbench',files(i).name);
        elseif endsWith(files(i).name,['.L.aparc.' resolution 'k_fs_LR.label.gii'])
    ...
    for i = 1:length(files)
        if endsWith(files(i).name,['.L.inflated.' resolution 'k_fs_LR.surf.gii'])
            filename = fullfile(mri_path,'workbench',files(i).name);
        end
    end
