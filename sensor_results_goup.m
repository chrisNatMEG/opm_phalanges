...
for i_peak = 1:length(params.peaks)
    ...
    snr.error_squidgrad = nan(n_subs,n_triggers);
    ...
    snr.prestim_squidgrad = nan(n_subs,n_triggers);
    ...
    snr.TFR_opm = nan(n_subs,n_triggers);
    snr.TFR_squidmag = nan(n_subs,n_triggers);
    snr.TFR_squidgrad = nan(n_subs,n_triggers);
    snr.TFR_squideeg = nan(n_subs,n_triggers);
    snr.TFR_opmeeg = nan(n_subs,n_triggers);
    snr.ratio_TFR = nan(n_subs,n_triggers);
    latency = [];
    ...
    for i_sub = subs
        ...
        for i_trigger = 1:n_triggers
            ...
            snr.error_squidgrad(i_sub,i_trigger) = peak_squidgrad{i_trigger}.peak_amplitude/peak_squidgrad{i_trigger}.std_error;
            ...
            snr.prestim_squidgrad(i_sub,i_trigger) = peak_squidgrad{i_trigger}.peak_amplitude/peak_squidgrad{i_trigger}.prestim_std;
            ...
            if isfield(params,'trigger_freq')
                snr.TFR_opm(i_sub,i_trigger) = TFR_opm{i_trigger}.SNR;
                snr.TFR_squidmag(i_sub,i_trigger) = TFR_squid{i_trigger}.SNR;
                snr.TFR_squidgrad(i_sub,i_trigger) = TFR_squidgrad{i_trigger}.SNR;
                snr.TFR_squideeg(i_sub,i_trigger) = TFR_squideeg{i_trigger}.SNR;
                snr.TFR_opmeeg(i_sub,i_trigger) = TFR_opmeeg{i_trigger}.SNR;
                snr.ratio_TFR(i_sub,i_trigger) = snr.TFR_opm(i_sub,i_trigger)/snr.TFR_squidmag(i_sub,i_trigger);
            end
...
%             close all
    ...
    if isfield(params,'trigger_freq')
        h = figure('DefaultAxesFontSize',16);
        bar(1:n_triggers,mean(snr.ratio_TFR,1,'omitnan'));
        hold on
        er = errorbar(1:n_triggers,mean(snr.ratio_TFR,1,'omitnan'), mean(snr.ratio_TFR,1,'omitnan')-min(snr.ratio_TFR,[],1,'omitnan'), max(snr.ratio_TFR,[],1,'omitnan')-mean(snr.ratio_TFR,1,'omitnan'));    
        er.Color = [0 0 0];                            
        er.LineStyle = 'none';  
        er.LineWidth = 1;
        er.CapSize = 30;
        p_values = zeros(1,n_triggers);
        for i = 1:n_triggers
            [~, p_values(i)] = ttest(snr.ratio_TFR(:, i)-1);
        end
        sigstar(arrayfun(@(x) [x, x], 1:n_triggers, 'UniformOutput', false), p_values);
        hold off
        title([params.peaks{1}.label ' SNR_{TFR} ratio (mean = ' num2str(mean(mean(snr.ratio_TFR,'omitnan'),'omitnan'),'%.2f') ')'])
        ylabel('OPM/SQUID')
        xlabel('Trigger')
        xticklabels(params.trigger_labels)
        saveas(h, fullfile(base_save_path, 'figs', ['SNR_ratios_TFR' peak_label '.jpg']))
        close all
    
        h = figure('DefaultAxesFontSize',16);
        bar(1:n_triggers,mean(snr.ratio_TFR,1,'omitnan'));
        hold on
        er = errorbar(1:n_triggers,mean(snr.ratio_TFR,1,'omitnan'), std(snr.ratio_TFR,0,1,'omitnan'), std(snr.ratio_TFR,0,1,'omitnan'));    
        er.Color = [0 0 0];                            
        er.LineStyle = 'none';  
        er.LineWidth = 1;
        er.CapSize = 30;
        ...
        xlabel('Trigger')
        xticklabels(params.trigger_labels)
        saveas(h, fullfile(base_save_path, 'figs', ['SNR_ratios_TFR2' peak_label '.jpg']))
        close all
        
        data = {snr.TFR_squidmag, snr.TFR_opm, snr.TFR_squidgrad};
        triggerLabels = params.trigger_labels;
        yLabelStr = 'SNR';
        titleStr = ['Group level ' params.peaks{1}.label ' SNR_{TFR} - SQMAG vs OPM vs SQGRAD'];
        save_path = fullfile(base_save_path, 'figs', ['SNR_TFR_box' peak_label '.jpg']);
