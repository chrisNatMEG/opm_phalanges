function create_group_report(folderPath, subs, params, i_peak)
...
reportName = ['group_report_' params.paradigm '_' peak_label];

% Create a new report
rpt = Report(fullfile(folderPath, reportName), 'pdf');
...
load(fullfile(base_save_path, ['group_sensor' peak_label]));
...
img = Image(fullfile(folderPath,'figs',['SNR_error_box.jpg']));
...
addGroupComparisonTables(section, dataCells, params.trigger_labels, {'squid', 'opm'}, '%.1f')
img = Image(fullfile(folderPath,'figs',['SNR_prestim_box.jpg']));
img.Style = {Width('14cm'), ScaleToFit};
add(section, img);
add(chapter, section);
...
addGroupComparisonTables(section, dataCells, params.trigger_labels, {'squid', 'opm'}, '%.1f')
img = Image(fullfile(folderPath,'figs',['Amplitude_meg_box.jpg']));
img.Style = {Width('14cm'), ScaleToFit};
add(section, img);
add(chapter, section);
...
section.Numbered = false; % Remove section numbering
dataCells = {};
dataCells{1} = snr.squidmag*1e3;
dataCells{2} = snr.opm*1e3;
addGroupComparisonTables(section, dataCells, params.trigger_labels, {'squid', 'opm'}, '%.1f')
img = Image(fullfile(folderPath,'figs',['Latency_box.jpg']));
img.Style = {Width('14cm'), ScaleToFit};
add(section, img);
add(chapter, section);
...
addGroupComparisonTables(section, dataCells, params.trigger_labels, {'squid', 'opm'}, '%.1f')
img = Image(fullfile(folderPath,'figs',['Ntrl_MEG_box.jpg']));
img.Style = {Width('14cm'), ScaleToFit};
add(section, img);
add(chapter, section);
...
for i_trigger = 1:length(params.trigger_labels)
    ...
    for i = 1:2
        ...
        for j = 1:2
            imgIndex = (j-1)*2 + i;
            img = Image(fullfile(folderPath,'figs',[sections2{imgIndex} '_grndAvg_butterfly_trig-' params.trigger_labels{i_trigger} '.jpg']));
...
for i_trigger = 1:length(params.trigger_labels)
    ...
    for i = 1:2
        ...
        for j = 1:2
            imgIndex = (j-1)*2 + i;
            img = Image(fullfile(folderPath,'figs',[sections2{imgIndex} peak_label '_grndAvg_topo_trig-' params.trigger_labels{i_trigger} '.jpg']));
            img.Style = {Width('8cm'), ScaleToFit};
            entry = TableEntry();
            append(entry, img);
            append(row, entry);
